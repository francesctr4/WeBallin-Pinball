...
PhysBody* Physics::CreateLeftLever(int x, int y, int width, int height, int angle, bodyType type)
{
	b2BodyDef body;
	...
	b2Vec2 vertices[4];
	vertices[0].Set(PIXEL_TO_METERS(-10), PIXEL_TO_METERS(-10));
	vertices[1].Set(PIXEL_TO_METERS(43), PIXEL_TO_METERS(-10));
	vertices[2].Set(PIXEL_TO_METERS(43), PIXEL_TO_METERS(6));
	vertices[3].Set(PIXEL_TO_METERS(-10), PIXEL_TO_METERS(6));
...
PhysBody* Physics::CreateRightLever(int x, int y, int width, int height, int angle, bodyType type)
{
	b2BodyDef body;
	...
	b2Vec2 vertices[4];
	vertices[0].Set(PIXEL_TO_METERS(45), PIXEL_TO_METERS(-6));
	vertices[1].Set(PIXEL_TO_METERS(-8), PIXEL_TO_METERS(-6));
	vertices[2].Set(PIXEL_TO_METERS(-8), PIXEL_TO_METERS(10));
	vertices[3].Set(PIXEL_TO_METERS(45), PIXEL_TO_METERS(10));
...
PhysBody* Physics::CreateCircleEnemy(int x, int y, int radious, bodyType type)
{
	b2BodyDef body;
...
bool Physics::PostUpdate()
{
		...
		for (b2Body* b = world->GetBodyList(); b; b = b->GetNext())
		{
			for (b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
			{
				...
				case b2Shape::e_circle:
				{
					...
					app->render->DrawCircle(METERS_TO_PIXELS(pos.x), METERS_TO_PIXELS(pos.y), METERS_TO_PIXELS(shape->m_radius) * app->win->GetScale(), 255, 0, 0);
				}
				break;
	...
	if (mouse_body != nullptr && mouse_joint != nullptr)
	{
		if (app->input->GetMouseButtonDown(SDL_BUTTON_LEFT) == KEY_REPEAT)
		{
			...
			app->render->DrawLine(METERS_TO_PIXELS(mouse_body->GetPosition().x), METERS_TO_PIXELS(mouse_body->GetPosition().y), app->input->GetMouseX(), app->input->GetMouseY(), 255, 0, 0);
		}
	}
